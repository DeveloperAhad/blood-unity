...
class Command(BaseCommand):
    def handle(self, *args, **options):  # sourcery skip: low-code-quality
        for division in divisions_en:
            division_query, state_created = Division.objects.get_or_create(
                name=division['title'],
                value=division['value']
            )
            if state_created:
                self.stdout.write(self.style.SUCCESS(f"✔ State or Division: {division['title']}"))
            else:
                self.stdout.write(f"✔ State or Division: {division['title']}")


        for district_key in districts_en:
            division = Division.objects.get(value=district_key)
            for district in districts_en[district_key]:
                district_query, district_created = District.objects.get_or_create(
                    name=district['title'],
                    value=district['value'],
                    division=division
                )
                if district_created:
                    self.stdout.write(self.style.SUCCESS(f"✔ District: {district['title']}"))
                else:
                    self.stdout.write(f"✔ District: {district['title']}")


        for upazila_key in upazillas_en:
            district = District.objects.get(value=upazila_key)
            for upazila in upazillas_en[upazila_key]:
                upazila_query, upazila_created = Upazila.objects.get_or_create(
                    name=upazila['title'],
                    value=upazila['value'],
                    district=district
                )
                if upazila_created:
                    self.stdout.write(self.style.SUCCESS(f"✔ Upazila: {upazila['title']}"))
                else:
                    self.stdout.write(f"✔ Upazila: {upazila['title']}")

        for union_key in unions_en:
            upazila = Upazila.objects.get(value=union_key)
            for union in unions_en[union_key]:
                union_query, union_created = Union.objects.get_or_create(
                    name=union['title'],
                    value=union['value'],
                    upazila=upazila
                )
                if union_created:
                    self.stdout.write(self.style.SUCCESS(f"✔ Union: {union['title']}"))
                else:
                    self.stdout.write(f"✔ Union: {union['title']}")
